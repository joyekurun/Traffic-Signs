...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        _temp = [jsonLoader tempFromJSONFile:url];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    vc.temp = [_temp objectAtIndex:indexPath.row];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MyCustomTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TemporaryCell"];
    
    List *temp = [_temp objectAtIndex:indexPath.row];
    
    cell.nameLabel.text = temp.label;
    cell.refLabel.text = temp.ref;
    
    [JSONLoader getImageData:temp.imageURL completionBlock:^(NSData* imageData)
     {
